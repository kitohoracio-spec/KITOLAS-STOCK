...
import { useFirestoreCollections } from '../hooks/useFirestoreCache'
...
function Skeleton({ h = 16, w = '100%' }) {
  return <div style={{ height: h, width: w, background: 'var(--surface2)', borderRadius: 6, animation: 'pulse 1.5s ease-in-out infinite' }} />
}
...
export default function Dashboard() {
  ...
  const { data, loading } = useFirestoreCollections({
    produtos: {},
    vendas: { orderByField: 'data', orderDir: 'desc', limitTo: 100 },
    ...(isAdmin ? {
      despesas: { orderByField: 'data', orderDir: 'desc', limitTo: 50 },
      fechos: { orderByField: 'data', orderDir: 'desc', limitTo: 20 },
    } : {})
  })
  ...
  const fechos = data.fechos || []

  const hasAnyData = produtos.length || vendas.length
      ...
      <>
        <div className="page-header">
          <div>
            ...
            <div className="page-sub">A carregar...</div>
          </div>
        </div>
        <div className="stat-grid">
          {[1,2,3,4].map(i => (
            <div className="stat-card" key={i}>
              <Skeleton h={36} w={36} />
              <div style={{marginTop:12}}><Skeleton h={11} w="60%" /></div>
              <div style={{marginTop:8}}><Skeleton h={24} w="80%" /></div>
            </div>
          ))}
        </div>
